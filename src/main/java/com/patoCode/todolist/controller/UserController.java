...
public class UserController {
    ...
    @PostMapping("/list/{idUser}")
    public ResponseEntity<String> addList(@PathVariable Long idUser, @RequestBody TodoList todoList){
        if(userServices.SaveTodoList(todoList, idUser)){
            return ResponseEntity.ok("Sucess!");
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Failed!");
    }
